...
async function loadServices() {
  ...
  try {
    ...
    if (!result.success) {
      servicesContainer.innerHTML = `<p class="loading-box">تعذر تحميل الخدمات</p>`;
      return;
    }
    ...
    if (services.length === 0) {
      servicesContainer.innerHTML = `<p class="loading-box">لا توجد خدمات متاحة حاليًا</p>`;
      return;
    }
    ...
    servicesContainer.innerHTML = `<p class="loading-box">تعذر الاتصال بالسيرفر</p>`;
  }
}
...
async function loadOffers() {
  ...
  try {
    ...
    if (!result.success) {
      offersContainer.innerHTML = `<p class="loading-box">تعذر تحميل العروض</p>`;
      return;
    }
    ...
    if (offers.length === 0) {
      offersContainer.innerHTML = `<p class="loading-box">لا توجد عروض متاحة حاليًا</p>`;
      return;
    }
    ...
    offersContainer.innerHTML = `<p class="loading-box">تعذر الاتصال بالسيرفر</p>`;
  }
}
...
if (branchSelect)      branchSelect.addEventListener("change", loadAvailableSlots);
if (preferredDateInput) preferredDateInput.addEventListener("change", loadAvailableSlots);
  ...
  bookingForm.addEventListener("submit", async function (e) {
    ...
    try {
      ...
      if (result.success) {
        bookingForm.reset();
        slotSelect.innerHTML = `<option value="">اختاري الموعد المتاح</option>`;
        showMessage("تم إرسال طلب الحجز بنجاح ✅ وسيتم التواصل معك قريباً");
      } else {
